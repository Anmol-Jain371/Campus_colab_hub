...
import { Bell, Sparkles, Megaphone, Trophy, PlusCircle, Search, Calendar, CheckCircle } from 'lucide-react';
  ...
  return (
    <section id="screen-home" className="screen active">
      ...
      <div className="quick-actions-bar">
        <div className="quick-action-btn" onClick={onOpenCreateProject}>
          <PlusCircle color="var(--accent)" />
          <span>List a Project</span>
        </div>
        <div className="quick-action-btn" onClick={() => navigateTo('discover')}>
          <Search color="var(--accent)" />
          <span>Find Teammates</span>
        </div>
        <div className="quick-action-btn" onClick={() => navigateTo('events')}>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {events.slice(0, 2).map(ev => (
                <div key={ev.id} className="event-card-compact" style={{ display: 'flex', gap: '12px', borderBottom: '1px solid var(--border)', paddingBottom: '12px' }}>
